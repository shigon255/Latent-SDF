...
'''
Trainer for Geo-NeuS
Note: this is not used in latent painting
'''
class GeoNeusTrainer:
    def __init__(self, conf_path, mode='train', case='CASE_NAME', is_continue=False, checkpoint=False, suffix=''):
        ...
        # Networks
        model_type = 'latent_model' if mode == 'latent_paint' else 'model'
        self.nerf_outside = NeRF(**self.conf[model_type + '.nerf']).to(self.device)
        self.color_network = RenderingNetwork(**self.conf[model_type + '.rendering_network']).to(self.device)
        ...
        if latest_model_name is not None:
            ...
            if mode == "latent_paint":
                if args.train_from_geo_neus:
                    self.load_latent_checkpoint(latest_model_name)
                else:
                    self.load_checkpoint_only_sdf(latest_model_name)
            else:
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(res_step)):
            ...
            if self.iter_step % self.save_freq == 0:
                if self.mode == "latent_paint":
                    self.save_latent_checkpoint()
                else:
    ...
    Project
    '''
    def load_checkpoint_only_sdf(self, checkpoint_name):
    ...
    def load_latent_checkpoint(self, checkpoint_name):
        checkpoint = torch.load(os.path.join(self.base_exp_dir, 'checkpoints', checkpoint_name), map_location=self.device)
        self.nerf_outside.load_state_dict(checkpoint['latent_nerf'])
        self.sdf_network.load_state_dict(checkpoint['latent_sdf_network_fine'])
        self.deviation_network.load_state_dict(checkpoint['latent_variance_network_fine'])
        self.color_network.load_state_dict(checkpoint['latent_color_network_fine'])
        self.optimizer.load_state_dict(checkpoint['latent_optimizer'])
        self.iter_step = checkpoint['latent_iter_step']

        logging.info('End')
    def save_latent_checkpoint(self):
        checkpoint = {
            'latent_nerf': self.nerf_outside.state_dict(),
            'latent_sdf_network_fine': self.sdf_network.state_dict(),
            'latent_variance_network_fine': self.deviation_network.state_dict(),
            'latent_color_network_fine': self.color_network.state_dict(),
            'latent_optimizer': self.optimizer.state_dict(),
            'latent_iter_step': self.iter_step,
        }

        os.makedirs(os.path.join(self.base_exp_dir, 'checkpoints'), exist_ok=True)
        torch.save(checkpoint, os.path.join(self.base_exp_dir, 'checkpoints', 'latent_ckpt_{:0>6d}.pth'.format(self.iter_step)))
    ...
    def render_latent_image(self, theta, phi, radius, resolution_level):
        ...
        img_fine = (np.concatenate(out_latent_fine, axis=0).reshape([H, W, 4]) * 256) # latent image
        return img_fine
